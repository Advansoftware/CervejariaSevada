...
<nav class="navbar navbar-inverse navbar-expand-md navbar-light bottom-box menuFixo">
  <a class="navbar-brand" href="<?=base_url();?>"><img src="<?=base_url();?>/content/imagens/LOGOMARCA.png" id="logo" class="large" alt="Cervejaria Sevda" title="Cervejaria Sevda"></a>
  ...
  <div class="navbar-offcanvas navbar-offcanvas-touch justify-content-end navbar-collapse" id="js-bootstrap-offcanvas">
    <ul class="navbar-nav mr-4" style="left: 100px;">
        ...
        <?php else : ?>
          <li class="nav-item">
        <?php endif;?>
          <a class="nav-link" href="<?=base_url();?>">Cervejaria<span class="sr-only">(current)</span></a>
      </li>
      <?php
        if($get_id==2):
        ?>
        ...
        <?php else : ?>
          <li class="nav-item">
        <?php endif;?>
        <a class="nav-link" href="<?=base_url();?>/produtos?id=2">Produtos</a>
      </li>
      <?php
      if($get_id==3):
        ?>
        ...
        <?php else : ?>
          <li class="nav-item">
        <?php endif;?>
        <a class="nav-link" href="<?=base_url();?>/visitacao?id=3">Visitação</a>
      </li>
      <?php
      if($get_id==4):
        ?>
        ...
        <?php else : ?>
          <li class="nav-item">
        <?php endif;?>
        <a class="nav-link" href="<?=base_url();?>/galeria?id=4">Galeria</a>
      </li>
      <?php
      if($get_id==5):
        ?>
        ...
        <?php else : ?>
          <li class="nav-item">
        <?php endif;?>
        <a class="nav-link" href="<?=base_url();?>/contato?id=5">Contato</a>
      </li>
	</ul>
  </div>
</nav>
